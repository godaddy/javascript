...
export function isLuhnValid({ cardNumber }: { cardNumber: string }) {
  ...
  for (let i = cleanedCardNumber.length; i--; i >= 0) {
    ...
    // biome-ignore lint/suspicious/noBitwiseOperators: needed for algorithm
    bit ^= 1;
...
export function getCardType({
  ...
  return Object.values(supportedCardProfiles).find(cardProfile => {
    const searchPattern = eager ? cardProfile.eagerPattern : cardProfile.pattern;
...
export function getCardProfile({
  ...
  if (!cardNumber && !cardType) throw new Error('Need at least cardNumber or cardType');

  cardType ??= cardNumber ? getCardType({ cardNumber, eager }) : undefined;

  return Object.values(supportedCardProfiles).find(cardProfile => cardProfile.id === cardType);
}
...
export function setupCardInfo({
  ...
  return {
    getCardType: ({ cardNumber, eager = false }: { cardNumber: string; eager?: boolean }) =>
      getCardType({ cardNumber, eager, supportedCardProfiles }),

    getCardProfile: ({ cardNumber, cardType, eager = false }: { cardNumber?: string; cardType?: CardType; eager?: boolean }) =>
      getCardProfile({ cardNumber, cardType, eager, supportedCardProfiles }),

    getCardGroups: ({ cardNumber }: { cardNumber: string }) => getCardGroups({ cardNumber, supportedCardProfiles }),
