...
export function DraftOrderTotals({
	...
	const handleDiscountsChange = (discounts: string[]) => {
		// This would typically update the order with the new discounts
		// For now this is just a placeholder that would be implemented later
		console.log("Discounts changed:", discounts);
	};
			...
			{enableDiscounts ? (
				<div className="mt-0">
					<DiscountStandalone
						...
						onError={(error) => console.error("Discount error:", error)}
					/>
				</div>
			) : null}
