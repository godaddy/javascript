...
export function ContactForm() {
	...
	useDraftOrderFieldSync({
		...
		mapToInput: (email) => {
			...
			const shippingIsDifferent = draftOrder?.shipping?.email !== email;
			const billingIsDifferent = draftOrder?.billing?.email !== email;

			return {
				...(shippingIsDifferent ? { shipping: { email: email?.trim() } } : {}),
				...(billingIsDifferent ? { billing: { email: email?.trim() } } : {}),
			};
		},
	});
